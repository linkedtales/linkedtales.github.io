const readProfiles = require('./readProfiles');

(async() => {


  const profiles = await readProfiles({ topCompany: 4 })
  //const p1 = profiles.filter(x => x.profile.summary)

  const words1 = profiles

  const words = words1.reduce((acc, { profile: { summary }}) => {
    if(summary){
      const a = summary.toLowerCase()
        .replace(/[,|.|\n|(|)|"|'|:|“|”|/]/g, '')
        .split(' ')

      acc = acc.concat(a)
    }

    return acc
  }, [])
  console.log(words)
  var counts = {};
  words.forEach((x) => { counts[x] = (counts[x] || 0)+1; });
  const xx = Object.keys(counts).map((key) => ({ word: key, count: counts[key]}))
    .sort((a, b) => {
      if(a.count > b.count) { return -1; }
      if(a.count < b.count) { return 1; }
      return 0;
    })

  //console.log(xx)

  const content = xx.reduce((acc, x) => {
    return acc = acc + `${x.word};${x.count}\n`
  }, '')



  // const content = profiles.reduce((acc,p) => {
  //   return acc = acc + `${p.profile.summary}\n`
  // }, '')

  const fs = require('fs')
  fs.writeFileSync('./oi.txt', content)
  //console.log(content)


})()
